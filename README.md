...
| 92~95 | 兼容旧板"继电器控制料车电机"协议（13/14主继电器对+15/16冗余复制，统一→M1），无实际GPIO，详见下方 |
...
**M1/M2 是同一台电机的双路硬件冗余**：按你的说明，M1、M2 两路 MOS 桥物理上
接的是同一台电机——哪路桥烧了，就把电机接线换到另一路，不用改程序。因此
两路必须始终收到完全相同的控制指令、受完全相同的限位保护，"两路同时驱动"
是本节的核心设计，不是可选项：

- **速度指令同步**：`mb_hook.c` 新增 `mb_hook_set_cart_speed()`，无论上位机
  写 `MOTOR_1_TARGET_SP_ADDR`、`MOTOR_2_TARGET_SP_ADDR`，还是走下面的
  OUT13~16 兼容协议，最终都会同时调用
  `API_MOTOR_SetSpeed(API_MOTOR_1,...)` 和 `API_MOTOR_SetSpeed(API_MOTOR_2,...)`，
  不存在只驱动其中一路的路径
- **限位/急停同步**：`API_MOTOR_CheckLimit()`（移植自旧板
  `SMD_CheckRelayMotorLimit()`，每 1ms 在 `api_smd.c` 的 TIM10 中断里调用）
  对 M1、M2 分别独立判断、独立停止，但用的是**同一组** IN 信号（沿用旧板
  编号，待你核实新板实际接线）：
  **IN8(索引7)/IN10(索引9)** 为正转方向限位，**IN9(索引8)** 为反转方向限位，
  **IN20(索引19)** 为全局急停。触发后立即调用 `API_MOTOR_Stop()` 停止对应
  一路——这是立即停止（两路PWM占空比直接置0），不走平滑减速斜坡，跟旧板
  继电器断电的效果一致

**兼容旧板"继电器控制料车电机"协议**：核对上位机 `worker/mainworker.cpp` 的
`onControlRelayMotor()` 后确认，`OUT_13_ADDR(92)`/`OUT_14_ADDR(93)` 是驱动
料车电机的主继电器对，`OUT_15_ADDR(94)`/`OUT_16_ADDR(95)` 是上位机对同一
信号的冗余复制（`CartUp`/`CartDown` 两个方向永远同时设置这 4 个寄存器，
13 与 15 同值、14 与 16 同值），**四个寄存器最终只对应同一台电机**，不是
两台（13=1,14=0，15/16同步→正转；13=0,14=1，15/16同步→反转；都为0→停止）。
新板没有对应的物理继电器，但为了让旧上位机代码不用改，在
`mbs_hook_extract_holding()` 里保留了这 4 个寄存器地址作为纯协议兼容层：
任意一个寄存器写入后都由 `mb_hook_decode_relay_pair()` 以 13/14 为准统一
解码，再通过 `mb_hook_set_cart_speed()` 同时驱动 M1+M2——效果等价于直接写
`MOTOR_1/2_TARGET_SP_ADDR`，同样会经过平滑加减速。上位机原有发
13/14/15/16 的代码完全不用改，新旧两套寄存器可以任选其一，互相兼容。
...
4. **SMD_EN 驱动使能引脚的有效电平未知**：目前固件完全不主动控制 EN（既不通过
